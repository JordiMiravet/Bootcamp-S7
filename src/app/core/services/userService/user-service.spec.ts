...
const mockCreateUser = jasmine.createSpy('createUserWithEmailAndPassword')
  .and.returnValue(Promise.resolve('usuario creado'));

const mockSignIn = jasmine.createSpy('signInWithEmailAndPassword')
  .and.returnValue(Promise.resolve('usuario logueado'));

const mockSignOut = jasmine.createSpy('signOut')
  .and.returnValue(Promise.resolve('usuario desconectado'));
...
describe('UserService', () => {
  ...
  it('login should call signInWithEmailAndPassword', async () => {
    const result = service.login({ email: 'test@test.com', password: '123456' });
